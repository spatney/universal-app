...
description: >
  ...
  Deploy with `rayfin up` for review. Orchestrates
...
in the deployed app. Build a thin vertical slice, preview it, then
...
6. **Drop the hero tile into `src/App.tsx`** — the scaffold renders a bundled
   ...
   providers + slicers + cross-filter wiring are the pattern to copy). Deploy
   with `rayfin up` for review.
